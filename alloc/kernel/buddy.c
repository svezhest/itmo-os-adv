...
int
bd_initfree_pair(int k, int bi, void *left_border, void *right_border) {
    int buddy = (bi % 2 == 0) ? bi+1 : bi-1;
    int free = 0;
    if(bit_isset(bd_sizes[k].alloc, bi / 2)) {
        // one of the pair is free
        free = BLK_SIZE(k);
//    free space is bounded between left and right
        if(left_border <= addr(k, buddy) && addr(k, buddy) <= right_border)
...
int
bd_initfree(void *bd_left, void *bd_right, void *end) {
    int free = 0;
    for (int k = 0; k < MAXSIZE; k++) {   // skip max size
        int left = blk_index_next(k, bd_left);
        int right = blk_index(k, bd_right);
        free += bd_initfree_pair(k, left, bd_left, end);
        if(left < right) {
            free += bd_initfree_pair(k, right, bd_left, end);
        }
    }
    return free;
}
...
bd_init(void *base, void *end) {
    ...
    int free = bd_initfree(p, bd_end, end);
